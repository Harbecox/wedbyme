...
class EntityRepository extends EntityResourceItem
{
    ...
    {
        $this->className = $this->name.( $this->isAdmin ? "Admin" : "")."Repository";
        $this->nameSpace = 'App\\Repositories\\'.( $this->isAdmin ? "Admin\\" : "").$this->className;
        $this->file = "app/Repositories/".( $this->isAdmin ? "Admin/" : "").$this->className.".php";
    }

    public function make(...$options)
    {
        $resource = $options[0];
        $dir = "app/Repositories/".( $this->isAdmin ? "Admin/" : "");
        if(!is_dir($dir)){
            mkdir($dir);
        }
        $data = "<?php\n";
        $data.= "\n";
        $data.= "namespace App\Repositories".( $this->isAdmin ? "\Admin" : "").";\n";
        $data.= "\n";
